...
def test_write_word_page_hides_heuristic_examples(tmp_path: Path) -> None:
    word = WordRecord(
        id=1,
        term="aber",
        display_prefix=None,
        ...
        part_of_speech=None,
        example_source='"aber" bedeutet "but".',
        example_target='"aber" means "but".',
        source="heuristic-enrichment-v3",
        tags="bulk-import,heuristic-enrichment",
        difficulty=None,
        times_shown=0,
        last_shown_at=None,
        created_at="",
    ...
    assert "Card Meaning" in html
    assert "but" in html
    assert "Example sentence" not in html
    assert "Example translation" not in html
    assert '"aber" bedeutet "but".' not in html
    assert 'id="pronounce-example-button"' not in html
